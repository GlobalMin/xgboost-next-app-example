...
from mongo_utils import (
    log_training_message, update_model_results, cleanup_failed_model
)
from modeling_utils import (
    load_dataset, preprocess_data, create_train_test_split,
    train_single_model, cross_validate_params, calculate_feature_importance,
    calculate_lift_chart, save_model, prepare_results, tune_hyperparameters
)
    ...
    try:
        ...
        if request.tune_parameters:
            ...
            best_params, cv_auc, best_n_estimators = tune_hyperparameters(
                dtrain, base_params, param_grid, 
                request.cv_folds, request.early_stopping_rounds, model_id
            )
            ...
            cv_auc, _, best_n_estimators = cross_validate_params(
                dtrain, final_params, request.cv_folds, 200, request.early_stopping_rounds
            )
            
            # Train final model
            model = train_single_model(
                dtrain, final_params, best_n_estimators,
                evals=[(dtest, 'eval')], 
        ...
        results = prepare_results(
            float(test_auc), cv_auc, feature_importance, 
            lift_data, best_params, best_n_estimators
        )
