...
def preprocess_data(df: pd.DataFrame, feature_columns: List[str], target_column: str) -> Tuple[pd.DataFrame, pd.Series, Dict[str, LabelEncoder]]:
...
def create_train_test_split(X: pd.DataFrame, y: pd.Series, test_size: float = 0.2, random_state: int = 42) -> Tuple[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]:
...
def train_single_model(dtrain: xgb.DMatrix, params: Dict[str, Any], num_rounds: int, 
                      evals: Optional[List] = None, early_stopping_rounds: Optional[int] = None) -> xgb.Booster:
...
def cross_validate_params(dtrain: xgb.DMatrix, params: Dict[str, Any], cv_folds: int, 
                         num_rounds: int, early_stopping_rounds: int) -> Tuple[float, float, int]:
...
def calculate_feature_importance(model: xgb.Booster, feature_names: List[str]) -> Dict[str, float]:
    """Extract feature importance from trained model"""
    # Try different importance types to get meaningful values
    importance_dict = model.get_score(importance_type='gain')  # Use 'gain' instead of 'weight'
    ...
    for feature in feature_names:
        ...
        if importance_value == 0.0:
            feature_index = feature_names.index(feature) if feature in feature_names else -1
            if feature_index >= 0:
...
def calculate_lift_chart(y_true: np.ndarray, y_pred_proba: np.ndarray, n_bins: int = 10) -> List[Dict[str, Any]]:
...
def prepare_results(test_auc: float, cv_auc: float, feature_importance: Dict[str, float],
                   lift_data: List[Dict], best_params: Dict[str, Any], n_estimators: int) -> Dict[str, Any]:
...
def tune_hyperparameters(dtrain: xgb.DMatrix, base_params: Dict[str, Any], param_grid: Dict[str, List],
                        cv_folds: int, early_stopping_rounds: int, model_id: str) -> Tuple[Dict[str, Any], float, int]:
    ...
    log_training_message(model_id, f"Testing {len(param_combinations)} parameter combinations")
